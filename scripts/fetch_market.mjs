...
for (const o of list) {
  ...
  if ((o.expiration_timestamp - now) / 86400000 < 8) continue; // слишком короткие серии пропускаем
...
const targets = { s30: 30, s60: 60, s90: 90, s180: 180 };
...
for (const s of Object.keys(targets)) {
  ...
  slots[s] = {
    ...
    expiry_ts: best,
    days: Math.round((best - now) / 86400000),
    mark_pct: Math.round((tk.mark_price || 0) * 10000) / 100, // % от цены BTC
